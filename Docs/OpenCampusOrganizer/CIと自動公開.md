...
CI定義・配布スクリプトだけのPRは、planジョブのPythonテストとactionlintによる構文検証で確認します。アプリ本体を変更していないため、そのPRでのFlutter解析・OSビルドは省略します。mainではワークフロー定義やFlutter導入処理の変更に対して全OSを検証し、配布する場合は署名や公開も含めた実際の経路を確認します。アプリ本体やAndroid・Windowsのテストの変更が一緒にある場合、この省略は適用しません。
