...
ワークフロー定義と検証済みの配布用PythonだけのPRは、planジョブのPythonテストとactionlintで確認し、Flutter解析・OSビルドを省略します。対象は `oco_ci.py`、`publish_release.py`、`audit_payload.py`、`build_android_ci.py` と、planで実行する `lint_workflows.sh` に限定します。Flutter導入スクリプト、Android起動テスト用シェル、新しい未検証のスクリプト、アプリ本体、ネイティブテストを含むPRは省略しません。mainではワークフロー定義の変更に対して全OSを検証し、配布する場合は署名や公開も含めた実際の経路を確認します。
...
AndroidはGradleの依存関係とテスト用システムイメージをキャッシュします。鍵・パスワード・アプリの設定や履歴はキャッシュに含めません。GitHubのキャッシュはブランチのアクセス範囲に従うため、初回やキャッシュ失効時には再ダウンロードが必要です。
...
タグを作る前に停止した場合は、CIスクリプトの修正後も未公開の番号を使用できます。mainで現在の番号がすべての既存バージョンタグより大きく、同名タグがない場合は、mainの修正コミットから再ビルドして自動公開します。アプリ本体を変更する場合の番号増加は引き続き必須です。
