...
void TestShortcuts() {
  ...
  for (int round = 0; round < 3; ++round) {
    for (int resource = 101; resource <= 105; ++resource) {
      ...
      Check(UpdateAppearanceShortcuts(directory, executable, resource), "Cannot retry same shortcut theme");
      for (const auto& path : {owned, foreign}) {
        ComPtr<IShellLinkW> link;
        ComPtr<IPersistFile> file;
  ...
  Check(UpdateAppearanceShortcuts(directory, executable, 105), "Unchanged read-only shortcut should succeed");
...
int main() {
  ...
  try {
    ...
    Messenger messenger;
    {
      ...
      for (int round = 0; round < 3; ++round) {
        for (int i = 0; i < 5; ++i) {
          ...
          Check(Property(window, PKEY_AppUserModel_RelaunchIconResource) ==
                    std::wstring(executable) + L",-" + std::to_wstring(101 + i), "DPI update changed taskbar metadata");
    ...
    std::cout << "PASS: 5 themes, repeated updates, notification order, shortcuts, DPI, refresh, minimized window, invalid theme, disposal\n";
