...
`logic.test.js`: 日付、任意位置ヘッダー、末尾区間、空欄、登録日保留、日付重複、処理上限、4,096通りの独立オラクル照合。

`workflow.test.js`: 読取り→確認→ロック→再読取→専用書式更新。対象外への復帰、元色と他ルールの保持、該当0件、冪等性、保護・結合、競合、復元失敗を確認。

`safety.test.js`: 構文、グローバル宣言、最小権限、禁止API、固定数式。
